...
const
  ...
  ERR_HOME_DIRECTORY_NOT_DETECTED = 14;
  ERR_CODEBLOCKS_INVALID_INSTALLATION_PATH = 15;
  ERR_PATCH_ERROR = 255;
...
type
  ...
  private
    fInternalIntegration: Boolean;
    fErrorMessages: TStringList;
    fNoticeMessages: TStringList;
    fVerbose: Boolean;
    ...
    procedure OnPatcherProcessTaskStatus(Sender: TObject; const Message: string);
    ...
    function GetMessages(Messages: TStringList): string;
    procedure WriteMessages(Messages: TStringList);
...
begin
  ...
  fNoticeMessages.Clear;
  Write(Message, '... ');
end;

procedure TCodeBlocksPatcherApplication.OnPatcherProcessTaskStatus(
  Sender: TObject; const Message: string);
begin
  fNoticeMessages.Add(Message);
end;
...
begin
  WriteBool(Success);
  WriteMessages(fErrorMessages);
  WriteMessages(fNoticeMessages);
end;
...
var
  FinalStr: string;
  FinalErrorExitCode: Integer;

begin
  if not fInternalIntegration then
    WriteLn;

  FinalStr := EmptyStr;
  FinalErrorExitCode := ERR_PATCH_ERROR;

  case CommandOperation of
    coInstall:
      begin
        FinalErrorExitCode := ERR_PATCH_INSTALL_FAILED;
        FinalStr := 'patched';
      end;
    coUninstall:
      begin
        FinalErrorExitCode := ERR_PATCH_UNINSTALL_FAILED;
        FinalStr := 'restored';
      end;
    coReinstall:
      begin
        FinalErrorExitCode := ERR_PATCH_REINSTALL_FAILED;
        FinalStr := 'reinstalled';
      end;
  end;

  if not Success then
    fProgramExitCode := FinalErrorExitCode;
...
function TCodeBlocksPatcherApplication.GetMessages(Messages: TStringList): string;
var
  i: Integer;
...
begin
  ...
  begin
    Line := Trim(Messages[i]);
    if not IsEmpty(Line) then
    begin
      Result := Result + Separator + Line;
      Separator := CODEBLOCKS_PATCHER_ERROR_SEPARATOR;
    end;
  end;
end;

procedure TCodeBlocksPatcherApplication.WriteMessages(Messages: TStringList);
var
  i: Integer;
  Line: string;
...
begin
  ...
  else if (Buffer = 'INTERNAL-INITIALIZE-PROFILES') then
    Result := coInternalInitializeProfiles;         // for DreamSDK Manager only
...
begin
  // Handle DreamSDK Manager integration
  fInternalIntegration := HasOption('internal-integration');

  // Display application banner
  fDisplayBanner := (not HasOption('n', 'no-logo')) and (not fInternalIntegration);
  ...
  ErrorMsg := CheckOptions(
    'hni:c:m:o:b:se',
    'help no-logo install-dir: config-files: home-dir: operation: backup-dir: show-splash verbose internal-integration'
  );
  ...
  begin
    DoErrorTerminate(ERR_NO_OPERATION_PASSED,
      Format('Invalid operation: %s', [OperationSwitch]));
    Exit;
  end;
  ...
  if not DirectoryExists(Patcher.Settings.HomeDirectory) then
  begin
    DoErrorTerminate(ERR_HOME_DIRECTORY_NOT_DETECTED, 'Home directory wasn''t detected.');
    Exit;
  end;
  ...
  begin
    DoErrorTerminate(ERR_CODEBLOCKS_RUNNING,
      'Code::Blocks is running, can''t continue.');
    Exit;
  end;
  ...
  // Check if Code::Blocks is installed on this machine
  if (not DirectoryExists(Patcher.Settings.InstallationDirectory)) then
  begin
    // If we try to install: reject it with a proper error message
    if (CommandOperation = coInstall) then
    begin
      DoErrorTerminate(ERR_CODEBLOCKS_INVALID_INSTALLATION_PATH,
        'Code::Blocks installation path doesn''t exist, can''t continue.');
      Exit;
    end;
  end;
  ...
  Patcher.OnProcessTaskStatus := @OnPatcherProcessTaskStatus;
  ...
  begin
    ...
    if (CommandOperation = coInstall) and (not Patcher.Ready) then
    begin
  ...
  else if (CommandOperation = coInternalInitializeProfiles) then
  begin
    InitializeCodeBlocksProfiles;
    WriteLn('Profiles initialization finished!');
  end;

  // Handle patcher errors if necessary
  if fInternalIntegration and (fErrorMessages.Count > 0) then
  begin
    DoErrorTerminate(ERR_PATCH_ERROR, GetMessages(fErrorMessages));
    Exit;
  end;
...
begin
  ...
  WriteLn(RowHeader, CODEBLOCKS_PATCHER_ERROR_TAG, Message);
  Terminate(AExitCode);
end;
...
begin
  ...
  begin
    ...
    if GetParam([coInstall, coUninstall, coReinstall, coPrintStatus, coInternalRefresh, coInternalInitializeProfiles],
...
{$IFDEF DEBUG}
      DebugLog('Setting up InstallationDirectory: ' + InstallationDirectory);
{$ENDIF}
    end;
...
{$IFDEF DEBUG}
      DebugLog('Setting up BackupDirectory: ' + BackupDirectory);
{$ENDIF}
    end;
...
{$IFDEF DEBUG}
      DebugLog('Setting up ConfigurationFileNames: ' + TempParam);
{$ENDIF}
    end;
...
begin
  ...
  fNoticeMessages := TStringList.Create;
...
begin
  ...
  try
    Application.Title := 'Code::Blocks Patcher for DreamSDK';
    Application.Run;
  finally
    Application.Free;
  end;
