...
type
  ...
  TTaskStatus = procedure(Sender: TObject;
    const Message: string) of object;
  ...
  private
    ...
    fError: TErrorEvent;
    fProcessTaskStatus: TTaskStatus;
    ...
    function ExecuteTask(const StepMessage: string;
      TaskFunction: TTaskProc): Boolean;
  ...
  protected
    ...
    procedure LogNotice(const Message: string);
...
{$ENDIF}
    ...
    property OnProcessTaskStatus: TTaskStatus
      read fProcessTaskStatus
      write fProcessTaskStatus;
...
{$IFNDEF LITE_VERSION}
  CODEBLOCKS_PATCHER_FILES: array [0..2] of TCodeBlocksPatcherFile = (
    ...
    (
      FileName: 'codeblocks-20.03-dreamsdk-addon-bin-x86.7z';
      ResourceName: 'PACKAGE2003X86';
      Version: cbv2003x86
    ),
    (
...
var
  ...
  var
    i: Integer;
    PatcherFileInfo: TCodeBlocksPatcherFile;

  begin
    CodeBlocksPatchFileName := EmptyStr;

    CodeBlocksVersion := GetCodeBlocksVersion(Settings.InstallationDirectory);

    i := Low(CODEBLOCKS_PATCHER_FILES);
    while (i <= High(CODEBLOCKS_PATCHER_FILES)) and not FileExists(CodeBlocksPatchFileName) do
    begin
      PatcherFileInfo := CODEBLOCKS_PATCHER_FILES[i];
      if (PatcherFileInfo.Version = CodeBlocksVersion) then
      begin
        CodeBlocksDetectedVersion := CodeBlocksVersionToString(PatcherFileInfo.Version);
        CodeBlocksPatchFileName := ExtractEmbeddedFileToWorkingPath(
          PatcherFileInfo.ResourceName,
          PatcherFileInfo.FileName
        );
      end;
      Inc(i);
    end;
...
begin
  Result := False;
  CodeBlocksVersion := cbvUndefined;
  CodeBlocksPatchFileName := EmptyStr;
  CodeBlocksDetectedVersion := EmptyStr;

  RetrieveCodeBlocksPatchFileInformation;
  if CodeBlocksDetectedVersion <> EmptyStr then
  begin
    LogNotice(Format('Detected Code::Blocks version: %s',
      [CodeBlocksDetectedVersion]));
    ...
    LogError('Unknown Code::Blocks version!');
{$ELSE}
begin
  Result := False;
{$ENDIF}
...
{$IFDEF DEBUG}
    WriteLn('Setting up Operation: ', fOperation);
{$ENDIF}
...
function TCodeBlocksPatcher.ExecuteTask(const StepMessage: string;
  TaskFunction: TTaskProc): Boolean;
begin
  ...
  if Assigned(fProcessTaskEnd) then
    fProcessTaskEnd(Self, StepMessage, Result);

  fExecuteResult := fExecuteResult and Result;
end;
...
procedure TCodeBlocksPatcher.LogNotice(const Message: string);
begin
  if Assigned(fProcessTaskStatus) then
    fProcessTaskStatus(Self, Message);
end;
...
begin
{$IFDEF DEBUG}
  DebugLog('>>> TaskPatchCodeBlocksConfiguration: '
    + IntToStr(Settings.ConfigurationFileNames.Count) + ' file(s) to patch...');
{$ENDIF}
  Result := True;
  ...
  begin
    ...
    begin
{$IFDEF DEBUG}
      DebugLog('  >>> Patching: "' + CodeBlocksConfigurationFileName + '"');
{$ENDIF}
...
begin
  ...
  begin
    // Restore original C::B files
    Result := RunCodeBlocksBackupRestore(cbRestore);

{$IFDEF ENABLE_CBTOOLS_SAVE_CB_VERSION}
    // Remove DreamSDK patched version file
    DeleteCodeBlocksVersionFileFromInstallationDirectory(
      Settings.InstallationDirectory);
{$ENDIF}
  end
  else
